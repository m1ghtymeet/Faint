...
namespace Faint
{
    ...
    void FileSystemUI::Draw()
    {
        if (ImGui::Begin("File Browser")) {
            ...
            if (ImGui::BeginChild("Wrapper", avail, true)) {
                avail.y = 30;
                if (ImGui::BeginChild("Path", avail)) {
                    float childHeight = avail.y;
                    float contentHeight = 24.0f;
                    float verticalOffset = (childHeight - contentHeight) * 0.5f;
                    ImGui::SetCursorPosY(verticalOffset);

                    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, { 2, 4 });
                    ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
                    ImGui::PushFont(UI::IconSolid);
                    ...
                    ImGui::SetNextWindowContentSize(ImVec2(0, 30.0f));
                    if (ImGui::BeginChild("pathBrowser", ImVec2((ImGui::GetContentRegionAvail().x - (numButtonAfterPathBrowser * buttonWidth * searchBarSize)) - 4.0, 30.0f))) {
                        ImGui::SetCursorPosY(verticalOffset);
                        for (int i = paths.size() - 1; i > 0; i--) {
                            if (i != paths.size())
                                ImGui::SameLine();
                            std::string pathLabel = (i == paths.size() - 1) ? "Project files" : paths[i]->Name;
    ...
    void FileSystemUI::DrawDirectory(Ref<Directory> directory, uint32_t drawId) {
        ...
        Ref<Texture> texture = AssetManager::LoadTexture("data/editor/icons/DirectoryIcon.png");
        ImGui::Image((ImTextureID)texture->GetID(), ImVec2(100, 100), ImVec2(0, 1), ImVec2(1, 0));
    ...
    void FileSystemUI::DrawFile(Ref<File> file, uint32_t drawId)
    {
        ...
        if (ImGui::BeginDragDropSource()) {
            ...
            if (fileExtension == ".model" || fileExtension == ".obj") {
                dragType = "_Model";
            }
            if (fileExtension == ".skinnedmodel" || fileExtension == ".fbx") {
                dragType = "_SkinnedModel";
            }
        ...
        Ref<Texture> textureImage = AssetManager::LoadTexture("data/editor/icons/FileIcon.png");
        ...
        if (fileType == FileType::Image) {
            ...
            textureImage = AssetManager::LoadTexture(path);
        }
