...
layout (binding = 5) uniform samplerCube ShadowCubeMapTexture;
...
float ShadowCalculation(vec3 worldPos, vec3 normal, vec3 lightDir) {
    vec4 lightSpaceMat = lightSpacePos * vec4(worldPos, 1.0);

    vec3 projCoords = lightSpaceMat.xyz / lightSpaceMat.w;
...
void main() {
	...
	//float z = texture(m_Depth, TexCoord).r;
	...
	for (int i = 0; i < lightCount; i++) {
		...
		float shadow = ShadowCalculation(WorldPos, normal, normalize(lightPos - WorldPos));
	...
	float depthValue = texture(ShadowMapTexture, TexCoord).r ;
