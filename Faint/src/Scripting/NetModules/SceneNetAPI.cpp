...
namespace Faint {
	...
	bool EntityHasComponent(int id, int componentType)
	{
		...
		{
		case PREFAB:				return entity.GetComponent<PrefabComponent>();
		case TRANSFORM:				return entity.GetComponent<TransformComponent>();
		case LIGHT:					return entity.GetComponent<LightComponent>();
		case CAMERA:				return entity.GetComponent<CameraComponent>();
		case AUDIO_EMITTER:			return entity.GetComponent<AudioComponent>();
		case MODEL:					return entity.GetComponent<MeshRendererComponent>();
		//case SKINNED_MODEL:			return entity.HasComponent<SkinnedModelComponent>();
		//case BONE:					return entity.HasComponent<BoneComponent>();
		case BOX_COLLIDER:			return entity.GetComponent<BoxColliderComponent>();
		case SPHERE_COLLIDER:		return entity.GetComponent<SphereColliderComponent>();
		...
		case CHARACTER_CONTROLLER:	return entity.GetComponent<CharacterControllerComponent>();
...
//		case QUAKE_MAP:				entity.AddComponent<QuakeMapComponent>(); break;
	...
	void TransformSetPosition(int entityID, float x, float y, float z) {
		...
		if (entity.GetComponent<TransformComponent>()) {
			...
			if (entity.GetComponent<CharacterControllerComponent>())
			{
	...
	Coral::Array<float> TransformGetRotation(int entityID) {
		...
		if (entity.GetComponent<TransformComponent>()) {
			...
			auto* component = entity.GetComponent<TransformComponent>();
			auto rotation = component->GetLocalRotation();
			Coral::Array<float> result = Coral::Array<float>::New({ rotation.x, rotation.y, rotation.z });
			return result;
		}
	}
	...
	void SceneNetAPI::RegisterMethods()
	{
		...
		RegisterMethod("CharacterControllerComponent.MoveIcall", &MoveAndSlide);
