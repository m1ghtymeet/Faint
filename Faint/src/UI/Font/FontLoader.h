...
namespace FaintUI {
	...
	public:
		...
		static bool makeAtlas(const std::vector<msdf_atlas::GlyphGeometry>& glyphs,
			...
			//font->mAtlas = CreateRef<Texture>(flags, Vec2(config.width, config.height), (void*)bitmap.pixels);
		...
		Ref<Font> LoadFont(const std::string& path)
		{
			...
			// Create atlas params
			msdf_atlas::DimensionsConstraint atlasSizeConstraint = msdf_atlas::DimensionsConstraint::MULTIPLE_OF_FOUR_SQUARE;
			...
			atlasPacker.setOuterPixelPadding(imageType == msdf_atlas::ImageType::MSDF || imageType == msdf_atlas::ImageType::MTSDF ? 0 : -1);
			...
			config.pxRange = atlasPacker.getPixelRange().lower;
