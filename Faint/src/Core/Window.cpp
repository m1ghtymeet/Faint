...
#define glCheckError() glCheckError_(__FILE__, __LINE__)
	...
	void Window::Init()
	{
		...
		const Vec2 defaultResolution = Vec2(1, 1);
		framebuffer = CreateRef<Framebuffer2>(true, defaultResolution);
		...
		framebuffer->SetTexture(outputTexture);
		framebuffer->SetTexture(CreateRef<Texture>(defaultResolution, GL_RED_INTEGER, GL_R32I, GL_INT), GL_COLOR_ATTACHMENT1); // Entity ID
	...
	void Window::Draw()
	{
		...
		Vec2 size = framebuffer->GetSize();
		size.x = std::max(size.x, 1.0f);
		size.y = std::max(size.y, 1.0f);

		if (Engine::IsPlayMode()) {
			scene->Draw(*framebuffer.get());
		}
		else {
			float resolutionScale = glm::clamp(Engine::GetProject()->Settings.ResolutionScale, 0.5f, 2.0f);
			scene->m_EditorCamera->SetViewportSize(size.x * resolutionScale, size.y * resolutionScale);
			scene->Draw(*framebuffer.get(), scene->m_EditorCamera->GetProjectionMatrix(), scene->m_EditorCamera->GetViewMatrix());
		}
	...
	//Ref<Framebuffer2> Window::GetFrameBuffer() const
	//{
	//	return framebuffer;
	//}
