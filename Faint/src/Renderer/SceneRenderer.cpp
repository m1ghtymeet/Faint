...
namespace Faint {
	...
	// GBuffer
	Scope<Framebuffer2> m_GBuffer;
	Scope<Framebuffer2> m_ShadingBuffer;
	Scope<Framebuffer2> m_ToneMapBuffer;
	Scope<Framebuffer2> m_OutlineBuffer;
	...
	void SceneRenderer::RenderScene(Scene& scene, Framebuffer2& framebuffer, bool renderUI) {
		...
		if (renderUI) {
			//m_GBuffer->QueueResize(framebufferResolution);
			g_frameBuffers["GBuffer"].Resize(framebufferResolution.x, framebufferResolution.y);
		}
		//std::cout << framebufferResolution.x << ", " << framebufferResolution.y << "\n";
		...
		if (renderUI) {
			//m_ShadingBuffer->QueueResize(framebufferResolution);
		}

		ShadingPass(scene, framebuffer.GetTexture());
		...
		//GLuint finalOutput = g_frameBuffers["GBuffer"].GetColorAttachmentHandleByName("Lighting");
		//framebuffer.Bind();
		//{
		//	RenderCommand::Clear();
		//	Shader* shader = ShaderManager::GetShader("copy");
		//	shader->Use();
		//
		//	//finalOutput.get()->Bind(0);
		//	glActiveTexture(GL_TEXTURE0);
		//	glBindTexture(GL_TEXTURE_2D, finalOutput);
		//
		//	Renderer::DrawQuad();
		//	shader->Stop();
		//}
		...
		//finalOutput = framebuffer.GetRenderID();

		//ProjectSettings projectSettings = Engine::GetProject()->Settings;
		//m_OutlineBuffer->QueueResize(framebufferResolution);
		//m_OutlineBuffer->Bind();
		//{
		//	RenderCommand::Clear();
		//	Shader* shader = ShaderManager::GetShader("outline");
		//	shader->Use();
		//	shader->SetInt("EntityID", m_hoveredEntityID == 1037621465 ? 1037621465 : m_hoveredEntityID + 1);
		//	shader->SetVec4("OutlineColor", projectSettings.PrimaryColor);
		//	shader->SetFloat("Radius", projectSettings.OutlineRadius * projectSettings.ResolutionScale);
		//
		...
		//
		//	m_GBuffer->GetTexture(GL_COLOR_ATTACHMENT3)->Bind(0);
		//	m_GBuffer->GetTexture(GL_DEPTH_ATTACHMENT)->Bind(1);
		//	m_GBuffer->GetTexture(GL_COLOR_ATTACHMENT7)->Bind(2);
		//
		//	Renderer::DrawQuad();
		//	shader->Stop();
		//
		...
		//framebuffer.Bind();
		////glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, GetGBuffer().GetTexture(GL_DEPTH_ATTACHMENT)->GetID(), 0);
		//{
		//	RenderCommand::Clear();
		//	Shader* shader = ShaderManager::GetShader("copy");
		//	shader->Use();
		...
		//	m_OutlineBuffer->GetTexture()->Bind(1);
		//
		//	Renderer::DrawQuad();
		//}
		//framebuffer.Unbind();
	...
	Framebuffer2* SceneRenderer::GetShadingBuffer() {
		return m_ShadingBuffer.get();
	}
