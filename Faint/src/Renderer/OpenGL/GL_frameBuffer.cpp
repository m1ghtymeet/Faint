...
namespace Moon {
    ...
    void FrameBuffer::CreateAttachment(const char* name, GLenum internalFormat, GLenum minFilter, GLenum magFilter) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, m_width, m_height, 0, colorAttachment.format, colorAttachment.type, nullptr);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, minFilter);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, magFilter);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);

        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + m_colorAttachments.size() - 1, GL_TEXTURE_2D, colorAttachment.id, 0);
        ...
        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
            std::cerr << "Framebuffer '" << m_name << "' is not complete after creating '" << name << "'!\n";
        }
    }
    ...
    void FrameBuffer::CreateDepthAttachment(GLenum internalFormat, GLenum minFilter, GLenum magFilter, GLint wrap, glm::vec4 borderColor) {
        ...
        GLenum format = GLInternalFormatToGLFormat(internalFormat);
        GLenum type = GLInternalFormatToGLType(internalFormat);
    ...
    void FrameBuffer::Resize(int width, int height) {
        ...
        for (size_t i = 0; i < m_colorAttachments.size(); ++i) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, colorAttachment.internalFormat,
                m_width, m_height, 0,
                colorAttachment.format, colorAttachment.type, nullptr);

            // TODO: Store min/mag filter & wrap if needed
        ...
        // Recreate depth attachment if it exists
        if (m_depthAttachment.id != 0) {
            glDeleteTextures(1, &m_depthAttachment.id);
            glGenTextures(1, &m_depthAttachment.id);
            glBindTexture(GL_TEXTURE_2D, m_depthAttachment.id);

            GLenum format = GLInternalFormatToGLFormat(m_depthAttachment.internalFormat);
            GLenum type = GLInternalFormatToGLType(m_depthAttachment.internalFormat);

            glTexImage2D(GL_TEXTURE_2D, 0, m_depthAttachment.internalFormat,
                m_width, m_height, 0, format, type, nullptr);
        ...
        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
            std::cerr << "Framebuffer '" << m_name << "' resize failed: incomplete framebuffer!" << std::endl;
        }
    ...
    void FrameBuffer::BlitToBackBuffer(const char* srcName, uint16_t backBufferWidth, uint16_t backBufferHeight) {

        GLint srcAttachmentSlot = GetColorAttachmentSlotByName(srcName);
    ...
    void FrameBuffer::BlitToDefaultFrameBuffer(const char* srcName, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, GLbitfield mask, GLenum filter) {
        ...
        if (srcAttachmentSlot != GL_INVALID_VALUE) {
            ...
            glReadBuffer(srcAttachmentSlot);
            glDrawBuffer(GL_BACK);
            glBlitFramebuffer(0, 0, GetWidth(), GetHeight(), dstX0, dstX1, dstY0, dstY1, mask, filter);
        }
    }
}