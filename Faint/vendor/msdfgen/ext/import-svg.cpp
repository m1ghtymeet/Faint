...
static double arcAngle(Vec2 u, Vec2 v) {
    return nonZeroSign(crossProduct(u, v))*acos(clamp(dotProduct(u, v)/(u.length()*v.length()), -1., +1.));
}

static Vec2 rotateVector(Vec2 v, Vec2 direction) {
    return Vec2(direction.x*v.x-direction.y*v.y, direction.y*v.x+direction.x*v.y);
}

static void addArcApproximate(Contour &contour, Point2 startPoint, Point2 endPoint, Vec2 radius, double rotation, bool largeArc, bool sweep) {
    ...
    Vec2 axis(cos(rotation), sin(rotation));

    Vec2 rm = rotateVector(.5*(startPoint-endPoint), Vec2(axis.x, -axis.y));
    Vec2 rm2 = rm*rm;
    ...
    Vec2 rc(q*radius.x*rm.y/radius.y, -q*radius.y*rm.x/radius.x);
    Point2 center = .5*(startPoint+endPoint)+rotateVector(rc, axis);

    double angleStart = arcAngle(Vec2(1, 0), (rm-rc)/radius);
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        controlPoint[0] = center+rotateVector(Vec2(d.x-cl*d.y, d.y+cl*d.x)*radius, axis);
        angle += angleIncrement;
        d.set(cos(angle), sin(angle));
        controlPoint[1] = center+rotateVector(Vec2(d.x+cl*d.y, d.y-cl*d.x)*radius, axis);
...
bool loadSvgShape(Shape &output, const char *filename, int pathIndex, Vec2 *dimensions) {
    ...
    Vec2 dims(root->DoubleAttribute("width"), root->DoubleAttribute("height"));
...
bool loadSvgShape(Shape &output, const char *filename, int pathIndex, Vec2 *dimensions) {
    ...
    Vec2 dims(pathAggregator.dimensions);
