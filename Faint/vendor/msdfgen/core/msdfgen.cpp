...
void generateSDF(const BitmapRef<float, 1> &output, const Shape &shape, Range range, const Vec2 &scale, const Vec2 &translate, bool overlapSupport) {
    generateSDF(output, shape, Projection(scale, translate), range, GeneratorConfig(overlapSupport));
}

void generatePSDF(const BitmapRef<float, 1> &output, const Shape &shape, Range range, const Vec2 &scale, const Vec2 &translate, bool overlapSupport) {
    generatePSDF(output, shape, Projection(scale, translate), range, GeneratorConfig(overlapSupport));
}

void generatePseudoSDF(const BitmapRef<float, 1> &output, const Shape &shape, Range range, const Vec2 &scale, const Vec2 &translate, bool overlapSupport) {
    generatePSDF(output, shape, Projection(scale, translate), range, GeneratorConfig(overlapSupport));
}

void generateMSDF(const BitmapRef<float, 3> &output, const Shape &shape, Range range, const Vec2 &scale, const Vec2 &translate, const ErrorCorrectionConfig &errorCorrectionConfig, bool overlapSupport) {
    generateMSDF(output, shape, Projection(scale, translate), range, MSDFGeneratorConfig(overlapSupport, errorCorrectionConfig));
}

void generateMTSDF(const BitmapRef<float, 4> &output, const Shape &shape, Range range, const Vec2 &scale, const Vec2 &translate, const ErrorCorrectionConfig &errorCorrectionConfig, bool overlapSupport) {
...
void generateSDF_legacy(const BitmapRef<float, 1> &output, const Shape &shape, Range range, const Vec2 &scale, const Vec2 &translate) {
    ...
    for (int y = 0; y < output.height; ++y) {
        ...
        for (int x = 0; x < output.width; ++x) {
            double dummy;
            Point2 p = Vec2(x+.5, y+.5)/scale-translate;
...
void generateMSDF_legacy(const BitmapRef<float, 3> &output, const Shape &shape, Range range, const Vec2 &scale, const Vec2 &translate, ErrorCorrectionConfig errorCorrectionConfig) {
    DistanceMapping distanceMapping(range);
#ifdef MSDFGEN_USE_OPENMP
    #pragma omp parallel for
#endif
...
void generateMTSDF_legacy(const BitmapRef<float, 4> &output, const Shape &shape, Range range, const Vec2 &scale, const Vec2 &translate, ErrorCorrectionConfig errorCorrectionConfig) {
    DistanceMapping distanceMapping(range);
#ifdef MSDFGEN_USE_OPENMP
    #pragma omp parallel for
#endif
