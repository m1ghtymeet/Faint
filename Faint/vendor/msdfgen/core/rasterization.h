...
void rasterize(const BitmapRef<float, 1> &output, const Shape &shape, const Vec2 &scale, const Vec2 &translate, FillRule fillRule = FILL_NONZERO);
void distanceSignCorrection(const BitmapRef<float, 1> &sdf, const Shape &shape, const Vec2 &scale, const Vec2 &translate, FillRule fillRule = FILL_NONZERO);
void distanceSignCorrection(const BitmapRef<float, 3> &sdf, const Shape &shape, const Vec2 &scale, const Vec2 &translate, FillRule fillRule = FILL_NONZERO);
void distanceSignCorrection(const BitmapRef<float, 4> &sdf, const Shape &shape, const Vec2 &scale, const Vec2 &translate, FillRule fillRule = FILL_NONZERO);
