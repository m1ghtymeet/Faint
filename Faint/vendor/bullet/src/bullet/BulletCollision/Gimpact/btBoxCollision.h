...
//	const btVec3 & edge,
//	const btVec3 & absolute_edge,
//	const btVec3 & pointa,
//	const btVec3 & pointb, const btVec3 & extend,
//	int dir_index0,
//	int dir_index1
...
//	const btVec3 & absolute_edge,
//	const btVec3 & pointa,
//	const btVec3 & pointb, btVec3 & extend)
//{
//
...
SIMD_FORCE_INLINE btScalar bt_mat3_dot_col(
	const btMatrix3x3 &mat, const btVec3 &vec3, int colindex)
{
...
{
public:
	btVec3 m_T1to0;    //!< Transforms translation of model1 to model 0
	...
	{
		//		static const btVec3 vepsi(1e-6f,1e-6f,1e-6f);
	...
	SIMD_FORCE_INLINE btVec3 transform(const btVec3 &point) const
	{
...
public:
	...
	{
		btVec3 center = (m_max + m_min) * 0.5f;
		btVec3 extends = m_max - center;
		// Compute new center
		center = trans(center);

		btVec3 textends = extends.dot3(trans.getBasis().getRow(0).absolute(),
	...
	SIMD_FORCE_INLINE void appy_transform_trans_cache(const BT_BOX_BOX_TRANSFORM_CACHE &trans)
	{
		...
		btVec3 textends = extends.dot3(trans.m_R1to0.getRow(0).absolute(),
	...
	SIMD_FORCE_INLINE void get_center_extend(btVec3 & center, btVec3 & extend) const
	{
	...
	*/
	SIMD_FORCE_INLINE bool collide_ray(const btVec3 &vorigin, const btVec3 &vdir) const
	{
		btVec3 extents, center;
	...
	SIMD_FORCE_INLINE void projection_interval(const btVec3 &direction, btScalar &vmin, btScalar &vmax) const
	{
		btVec3 center = (m_max + m_min) * 0.5f;
		btVec3 extend = m_max - center;
	...
	{
		//Taken from OPCODE
		btVec3 ea, eb;  //extends
		btVec3 ca, cb;  //extends
		...
		const btVector4 &triangle_plane) const
	{
		...
		btVec3 center, extends;
		this->get_center_extend(center, extends);

		const btVec3 v1(p1 - center);
		const btVec3 v2(p2 - center);
		const btVec3 v3(p3 - center);

		//First axis
		btVec3 diff(v2 - v1);
		btVec3 abs_diff = diff.absolute();
		//Test With X axis
